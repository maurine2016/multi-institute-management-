 ...
 *    $checker = new WL_MIM_LicenseChecker($_GET['license_key']);
 ...
 *     $checker = new WL_MIM_LicenseChecker(file_get_contents('license_key.php'));
...
class WL_MIM_LC
{
    ...
    /** @var WL_MIM_LicenseChecker_ActivationResponse */
     ...
     * @return \WL_MIM_LicenseChecker_ActivationResponse
     */
    protected function processActivationResponse($resp)
    {
        $response = new WL_MIM_LicenseChecker_ActivationResponse($resp);
    ...
    {
        $ret = new WL_MIM_LicenseChecker_ActivationResponse;
        if (empty($response))
            return $ret;
        $c = new WL_MIM_Crypt_Blowfish($this->_local_encryption_key);
        try {
...
class WL_MIM_LicenseChecker_ActivationResponse
{
    public $code;
    public $message;
...
class WL_MIM_Crypt_Blowfish
{
    ...
    function _init()
    {
        $defaults = new WL_MIM_Crypt_Blowfish_DefaultKey();
        $this->_P = $defaults->P;
        $this->_S = $defaults->S;
    }
...
class WL_MIM_Crypt_Blowfish_DefaultKey
{
    var $P = array();
