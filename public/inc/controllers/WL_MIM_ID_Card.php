...
class WL_MIM_ID_Card {
	...
	public static function get_id_card() {
		...
		if ( $data_of_birth_required ) {
			$row = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wl_min_students WHERE is_deleted = 0 AND enrollment_id = $student_id AND institute_id = $institute_id AND date_of_birth = '$date_of_birth'" );
		} else {
			$row = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wl_min_students WHERE is_deleted = 0 AND enrollment_id = $student_id AND institute_id = $institute_id" );
		}
		...
		?>
        <a class="ml-2 mb-3 btn btn-outline-primary" href="#wlmim-print-student" data-keyboard="false" data-backdrop="static" data-toggle="modal">
        	<?php esc_html_e( 'Print ID Card', WL_MIM_DOMAIN ); ?>&nbsp;<i class="fa fa-print"></i>
        </a>
		<!-- print student modal -->
		<div class="modal fade" id="wlmim-print-student" tabindex="-1" role="dialog" aria-labelledby="print-student-label" aria-hidden="true">
				            ...
				            <div class="col">
				                <div class="mb-3 mt-2">
				                    <div class="text-center">
				                        <button type="button" id="wlmim-id-card-print-button" class="btn btn-sm btn-success">
